...
function rosterRoutes(kind: 'player' | 'coach', table: typeof players | typeof coaches) {
  ...
  app.post(`/api/${resource}`, auth, async (req: AuthedRequest, res) => {
    ...
    if (!fullName.trim()) return fail(res, 400, 'Nome completo é obrigatório.')
    // Documento é opcional: quando informado, ainda vale a checagem de duplicidade entre times;
    // sem documento, não há o que comparar, então nenhuma entrada é criada em registeredDocuments.
    const normalized = normalizeDocument(document)
    try { const item = await db.transaction(async (tx) => { const active = await tx.select().from(tableAny).where(and(eq(tableAny.teamId, teamId), eq(tableAny.active, true))); if (active.length >= limit) throw new Error('LIMIT'); const [created] = await tx.insert(tableAny).values({ teamId, fullName: normalizeName(fullName), document: String(document).trim(), documentNormalized: normalized }).returning(); if (normalized) await tx.insert(registeredDocuments).values({ documentNormalized: normalized, teamId, kind, recordId: created.id }); return created }); res.status(201).json(item) } catch (error) { return fail(res, 409, error instanceof Error && error.message === 'LIMIT' ? `LIMITE DE ${limit} ATINGIDO.` : 'Este documento já está cadastrado.') }
  })
  ...
  app.delete(`/api/${resource}/:id`, auth, async (req: AuthedRequest, res) => { const [item] = await db.select().from(tableAny).where(eq(tableAny.id, param(req, 'id'))); if (!item) return fail(res, 404, 'Registro não encontrado.'); if (!ownTeam(req, res, item.teamId)) return fail(res, 403, 'Sem acesso.'); await db.transaction(async (tx) => { await tx.update(tableAny).set({ active: false }).where(eq(tableAny.id, item.id)); if (item.documentNormalized) await tx.delete(registeredDocuments).where(eq(registeredDocuments.documentNormalized, item.documentNormalized)) }); res.status(204).end() })
}
